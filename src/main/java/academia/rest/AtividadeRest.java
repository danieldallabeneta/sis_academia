...
public class AtividadeRest {
    ...
    public ModelAtividade getAtividadeById(@PathVariable int id) throws Exception {
        Optional<ModelAtividade> equip = repo.findById(id);
        if (equip.isEmpty()) {
            return null;
        } else {
            return equip.get();
    ...
    public void updateAtividade(@Valid @RequestBody ModelAtividade ativ) {
        Optional<ModelAtividade> aux = repo.findById(ativ.getId());
